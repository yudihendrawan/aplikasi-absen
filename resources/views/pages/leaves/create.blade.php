        ...
        <div class=" ">
            ...
            <form action="{{ route('leaves.store') }}" method="POST"
                ...
                <div class="mb-4 md:col-span-2">
                    <label for="user_id"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('User') }}</label>
                    <select id="user_id" name="user_id" required
                        class="tom-select @error('user_id') border-red-500 @enderror">
                        <option value="" disabled {{ old('user_id') ? '' : 'selected' }}>{{ __('Pilih pegawai') }}
                        </option>
